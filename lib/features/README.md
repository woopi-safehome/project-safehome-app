...
화면 하나가 폴더 하나다. 공통 인프라(`lib/core/`)에 의존하며, 그 반대는 없다.
...
**화면 하나에 폴더 하나**를 만들고, 그 안에 화면과 — 필요하면 — 상태를 함께 둔다.

- **상태가 필요한 화면만 상태 파일을 만든다.** 입력도 통신도 없는 안내성 화면은 화면 파일 하나로 끝난다.
- **상태가 둘 이상일 수 있다.** 한 화면이 성격이 다른 상태를 다루면 나눠서 둔다.
- **그 화면에서만 쓰는 위젯은 폴더 안에 함께 둔다.** 다른 화면이 쓰기 시작할 때 밖으로 옮긴다.

만든 뒤 **라우터에 등록해야 비로소 라우트가 생긴다.**
...
정상 완료, 실패 이벤트, **스트림의 비정상 종료**. 셋 다 테스트가 잡는다. 세 번째를 빼먹으면
...
**정의처가 하나가 아니다** — 응답에 실려 오는 값은 직렬화 모델이, **에러 코드는 예외 타입이** 갖는다.
어느 쪽이든 **화면에서 문자열로 쓰지 않는다.** 제약 테스트가 두 정의처의 어휘를 모아
바깥에서 쓰인 곳을 찾는다. 정의처가 늘면 **그 테스트에도 함께 올려야** 새 어휘가 검사 대상이 된다.

**에러 코드는 모르는 값이 와도 동작해야 한다.** 서버가 앱보다 먼저 배포되므로
새 코드가 먼저 도착할 수 있다. 알아보지 못한 코드는 일반 실패로 다룬다.
